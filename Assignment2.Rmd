...
```{r}
reducedDf<- stormData[stormData$years >= 1996,]
head(reducedDf)
```
...
```{r}
fatalInjury<-subset(reducedDf,FATALITIES > 0 & INJURIES > 0);dim(fatalInjury)
fatalInjury1<-subset(reducedDf,FATALITIES == 0 & INJURIES == 0);dim(fatalInjury1)

asd<-reducedDf[reducedDf$FATALITIES == 0 &  reducedDf$INJURIES == 0,]
 dim(asd)    
dim(reducedDf)-dim(fatalInjury1)



class(reducedDf$FATALITIES)
head(asd,50)
head(stormData)
 names(stormData)
str(stormData)
